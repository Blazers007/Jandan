...
public class FragmentNestedScrollBehavior extends CoordinatorLayout.Behavior<FrameLayout> {
    ...
    public void onNestedScroll(CoordinatorLayout coordinatorLayout, FrameLayout child, View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        View bottonNavitationView = coordinatorLayout.findViewById(R.id.bottom_navigation_view_wrapper);
        if (dyConsumed > 0) {
            bottonNavitationView.animate().translationY(bottonNavitationView.getHeight()).setDuration(500).setListener(mListener).start();
        } else {
            bottonNavitationView.animate().translationY(0).setDuration(500).setListener(mListener).start();
        }
    }
