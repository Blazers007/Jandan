...
public class WatchRelativeLayout extends RelativeLayout implements INightWatch {
    ...
    void init(Context context, AttributeSet attrs) {
        if (null == attrs)
            return;
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.WatchRelativeLayout, 0, 0);
        try {
            /* 一般 */
            bgDay = a.getDrawable(R.styleable.WatchRelativeLayout_wtg_day_bg);
